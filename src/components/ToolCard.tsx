...
export default function ToolCard({ tool }: { tool: Tool }) {
  return (
    <Link to={`/tools/${tool.slug}`} className="card group flex flex-col p-6 transition hover:border-line-strong hover:bg-surface">
      <div className="flex items-center justify-between">
        <FieldBadge field={tool.field} />
        <ArrowUpRight className="h-4 w-4 text-faint transition group-hover:text-fg" />
      </div>
      <h3 className="mt-6 text-xl">{tool.name}</h3>
